...
if [[ "$PUBLISH" == false ]]; then
  ...
  {
    echo "# Release draft: DotZap $VERSION"
    echo ""
    ...
    echo "    > - **Already installed?** DotZap auto-updates — pick *Check for Updates…* in the About tab to grab it now, or wait for the next scheduled check."
    echo "    > - **First time?** Download \`$DMG\`, open it, drag DotZap to Applications. First launch: right-click → Open, then System Settings → Privacy & Security → Open Anyway. Then grant Full Disk Access via the banner in the Volumes tab."
...
Now ask Claude:
  ...
  DotZap $VERSION into RELEASE_NOTES.md. Follow the
...
git tag -a "$TAG" "$COMMIT" -m "DotZap $VERSION"
...
ASSET_URL="https://github.com/Foiler25/DotZap/releases/download/${TAG}/${DMG}"
